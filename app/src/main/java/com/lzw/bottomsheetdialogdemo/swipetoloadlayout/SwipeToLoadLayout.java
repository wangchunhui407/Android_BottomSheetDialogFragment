...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    private boolean isLoading, isAutoLoadMore;
    /**
     * dispatchTouchEvent ActionDown事件
     */
    private ActionDownCallback callback;
...
//        }
    ...
    public ListViewOnScrollListener listViewOnScrollListener;

    public void setListViewOnScrollListener(ListViewOnScrollListener listViewOnScrollListener) {
        this.listViewOnScrollListener = listViewOnScrollListener;
    }
...
//                    Log.d("OnScrollListener", "lastPosition --> " + lastPosition);
//                    Log.d("OnScrollListener", "itemCount  --> " + totalItemCount);
//                    Log.d("OnScrollListener", "isLoading  --> " + isLoading);
                    //如果当前不是正在加载更多，并且到了该加载更多的位置，加载更多。
                    if (lastPosition != -1 && !isLoading && lastPosition + 3 >= totalItemCount) {
                        if (mLoadMoreEnabled && mLoadMoreCallback != null) {
                            isLoading = true;
                            setStatus(STATUS.STATUS_LOADING_MORE);
                            mLoadMoreCallback.onLoadMore();
//                            Log.d("OnScrollListener", "onLoadMore------------------------");
                        }
                    }
                }
...
//                    int lastPosition = layoutManager.findLastVisibleItemPosition();
//
////                    Log.d("OnScrollListener", "onScrolled");
////                    Log.d("OnScrollListener", "lastPosition --> " + lastPosition);
////                    Log.d("OnScrollListener", "itemCount  --> " + totalItemCount);
////                    Log.d("OnScrollListener", "isLoading  --> " + isLoading);
//                    //如果当前不是正在加载更多，并且到了该加载更多的位置，加载更多。
//                    if (lastPosition != -1 && !isLoading && lastPosition + 1 >= totalItemCount) {
//                        if (mLoadMoreEnabled && mLoadMoreCallback != null) {
//                            isLoading = true;
//                            setStatus(STATUS.STATUS_LOADING_MORE);
//                            mLoadMoreCallback.onLoadMore();
...
//                    Log.d("OnScrollListener", "onScrolled");
//                    Log.d("OnScrollListener", "lastPosition --> " + lastPosition);
//                    Log.d("OnScrollListener", "itemCount  --> " + view.getCount());
//                    Log.d("OnScrollListener", "isLoading  --> " + isLoading);

                    if (lastPosition != -1 && !isLoading && lastPosition + 1 >= view.getCount()) {
                        if (mLoadMoreEnabled && mLoadMoreCallback != null) {
                            isLoading = true;
                            setStatus(STATUS.STATUS_LOADING_MORE);
                            mLoadMoreCallback.onLoadMore();
    ...
    public void setRefreshHeaderView(View view) {
        if (view instanceof SwipeRefreshTrigger) {
            ...
            if (mHeaderView != view) {
                ...
                layoutParams.height = ((SwipeRefreshTrigger)view).refreshHeaderHeight();
//                addView(view, 0);
    ...
    private void layoutChildren() {
        ...
        if (mTargetView != null) {
            ...
            try {
                ...
                Log.i("SwipeToLoadLayout","exception:",t);
            }
        }
    ...
    public void setCallback(ActionDownCallback callback){
